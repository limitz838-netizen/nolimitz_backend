...
async def process_open_execution(db: Session, service: MetaApiService, trade: TradeExecution):
    try:
        ...
        for license_row in licenses:
            ...
            try:
                ...
                if start_time:
                    ...
                    if trade_time and start_time:
                        if trade_time.replace(tzinfo=timezone.utc) < start_time.replace(tzinfo=timezone.utc):
                            continue
                ...
                for _ in range(opens_to_make):
                    try:
                        ...
                        if trade.retry_count >= 3:
                            ...
                            trade.status = "retry"
                            delay_seconds = 5 * trade.retry_count
                            trade.retry_at = utc_now() + timedelta(seconds=delay_seconds)

                            trade.error_message = f"Retry {trade.retry_count}: {str(e)}"

                        db.commit()
                        continue  # 🔥 IMPORTANT → stop further execution
        ...
        # only mark failed if retries exhausted
            if (trade.retry_count or 0) >= 3:
                trade.status = "failed"
                trade.error_message = "No users received trade after retries"
            else:
                trade.status = "retry"
...
async def process_trade_safe(trade_id: int, service: MetaApiService):
    ...
    try:
        ...
        logger.error(f"SAFE WORKER ERROR trade_id={trade_id}: {e}")
        ...
        try:
            ...
            pending_trades = (
                ...
                .filter(
                    ...
                    (TradeExecution.retry_count.is_(None)) | (TradeExecution.retry_count < 3),
                    or_(
                        TradeExecution.retry_at.is_(None),
                        TradeExecution.retry_at <= now
                    )
                )
            ...
            for trade in pending_trades:
                tasks.append(process_trade_safe(trade.id, service))
