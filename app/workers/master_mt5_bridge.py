...
def safe_post(url, payload, retries=3, timeout=12):
    for attempt in range(retries):
        try:
            ...
            logger.warning(
                f"POST exception Attempt {attempt + 1}/{retries}: {e}"
            )
...
def main():
    ...
    try:
        ...
        logger.info(f"Backend health check → {r.status_code}")
        ...
        while True:
            try:
                ...
                if not mt5.terminal_info():
                    ...
                    if not mt5.initialize(path=MT5_PATH):
                        current_error = str(mt5.last_error())

                        if current_error != last_mt5_error:
                            logger.error(f"positions_get() failed: {current_error}")
                            last_mt5_error = current_error
                        time.sleep(10)
                        continue
                ...
                for pos in positions:
                    ...
                    if ticket in seen_positions:

                        old = seen_positions[ticket]

                        if (
                            old.get("sl") != pos_dict["sl"] or
                            old.get("tp") != pos_dict["tp"]
                        ):

                            logger.info(
                                f"Position modified → Ticket={ticket}"
                            )
